...
class AgentsController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        try {          
         ...
         return $this->sendErrorResponse('Fail to update agent'.$ex);
     }
    ...
    public function pdf_allagent(Request $request)
    {
            $html = $this->allpassager();
            $pdf = \App::make('dompdf.wrapper');
            // echo($html);
            $pdf->loadHTML($html);
            $pdf->loadHTML($html)->setPaper('a3');
    ...
    public function allpassager(){
        ...
        $data= $data  = DB::table('agents')->orderBy('id','asc')
         ->select("id","nomsag","genreag","datenaissag",
            "mobile","emailag",
             )  
        ->selectRaw('TIMESTAMPDIFF(YEAR, datenaissag, CURDATE()) as age_agent')
        ->get();
        $output='';
        $donnees='';
        foreach ($data as $rows){
            ...
            $genre=$rows->genreag;
            $age=$rows->age_agent;
            $datenaiss=$rows->datenaissag;
            $telephone=$rows->mobile;
            $email=$rows->emailag; 
                                            
     $donnees .='                
      <tr style="vertical-align:top;">
      <td style="height:41px;"></td>
      <td class="csAA5B9630" style="width:64px;height:39px;line-height:18px;text-align:center;vertical-align:top;"><nobr>'.$numero.'</nobr></td>
